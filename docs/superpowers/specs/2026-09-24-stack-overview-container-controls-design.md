...
- Keep the page header's stack-level **Stop**, **Restart**, **Validate**, and **Deploy** actions. **Stop** and **Restart** continue to act on the whole stack when its existing runtime conditions allow them. Deploy creates containers when none exist.
...
Extend the existing Compose-backed runtime with targeted Docker SDK start, stop, and restart calls by container ID. The SDK client already backs Compose in this process; use that connection rather than adding a CLI command or a second Docker connection. The operation acts on exactly the validated ID. The page header's stack-wide actions continue to use their existing backend actions.
...
- Frontend tests cover each action rule, distinct replica rows, empty and read-error states, archived and active-operation gating, polling updates, removal of the four Overview buttons and old status output, and continued availability of the header's whole-stack Stop and Restart actions.
